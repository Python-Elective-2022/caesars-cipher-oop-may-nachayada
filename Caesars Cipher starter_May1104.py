...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for letter in lowercase_letters:
            new_dict[letter] = lowercase_letters[(lowercase_letters.index(letter) + shift) % 26]
    
        for letter in uppercase_letters:
            new_dict[letter] = uppercase_letters[(uppercase_letters.index(letter) + shift) % 26]
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for number in range(26):
            decrypted_word = self.apply_shift(26-number)
            decrypted_word_list = decrypted_word.split()
            counter = 0
            for word in decrypted_word_list:
                if is_word(self.valid_words, word):
                    counter += 1
            if counter == len(decrypted_word_list):
                return (number, decrypted_word)
